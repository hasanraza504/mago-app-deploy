...
|---|---|
| `SOURCE_REPO` | `InstaFarms/Mago-app` |
...
| Name | What it is |
|---|---|
| `SOURCE_REPO_SSH_KEY` | Private half of a **read-only deploy key** on `InstaFarms/Mago-app` |
...
on `InstaFarms/Mago-app` with **write access disabled**. This is deliberately preferred
...
Revoke or rotate it under `InstaFarms/Mago-app` → Settings → Deploy keys.
